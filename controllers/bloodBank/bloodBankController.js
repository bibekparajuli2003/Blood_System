...
exports.renderHospitalDashboard = async (req, res, next) => {
  ...
  var ambulances = await sequelize.query(
    `SELECT * FROM ambulances WHERE hospitalId = ?`,
    {
      type: QueryTypes.SELECT,
      replacements: [id],
    }
...
exports.renderAddAmbulance = async (req, res, next) => {


  res.render("bloodBank/createAmbulance");
};
...
exports.addAmbulance = async (req, res, next) => {
  try {
    ...
    const [data] = await sequelize.query(`SELECT * FROM bloodBank WHERE hospitalId='${hospitalId}'`,{
      type  : QueryTypes.SELECT
    })
    ...
    await sequelize.query(
      ...
      {
        type: QueryTypes.INSERT,
        replacements: [hospitalId,name,phone,data.province,data.district,data.localLevel],
      }
    );
    res.redirect(`/bloodBank/dashboard/${hospitalId}`);
  } catch (error) {
